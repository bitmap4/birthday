(() => {
  ...
  const photos = Array.from(
    { length: 8 },
    (_, i) =>
      `<span class="instant-photo photo-${i + 1}"><img alt="" loading="lazy"></span>`,
  ).join("");
...
to see you!</p>
  ...
  const wideDefaults = {
    ...
    "wide-inspo-paper": { x: "63px", y: "524px", z: "1", r: "-1deg", w: "585px", h: "693px", edgeSeed: 20261204, edgeStyle: "separator" },
  ...
  fetch(pinterestApi + "?url=" + encodeURIComponent(board), { cache: "no-store" })
    ...
    .catch(() => {
      /* Leave the collage empty if live Pinterest metadata is unavailable. */
    });
  ...
  function renderRule(canvas) {
    ...
    ctx.lineWidth = canvas.closest(".wide-rule") ? 6.7 : 0.82;
    ctx.lineCap = "round";
    ctx.stroke();
  }
